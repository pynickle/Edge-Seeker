...
export async function decreaseItemQuantity(
    ...
    const userItem = await ctx.database
...
export async function addOrUpdateBuffEffect(
    ctx: Context,
    userId: string,
    buffType: BuffType,
    durationDays: number,
    ...
    // 查找用户当前有效的 buff 效果
    const activeEffects = await ctx.database
        .select('user_buff_effects')
        .where({
            userId,
            buffType,
...
// 计算用户当前 buff 的剩余天数
export async function getBuffRemainingDays(
    ctx: Context,
    userId: string,
    buffType: BuffType
): Promise<number> {
...
export async function hasActiveBuff(
    ...
    const effects = await ctx.database
        .select('user_buff_effects')
        .where({
            userId,
            buffType,
