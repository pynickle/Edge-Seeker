...
async function addReaction(session: Session, ctx: Context, messageId: number | string, emojiId: string) {
    await session.onebot._request('set_msg_emoji_like', {message_id: messageId, emoji_id: emojiId});
}

export async function stickEmoji(session: Session, ctx: Context, emojis: string[]) {
    try {
        const targetId = session.messageId;

        for (const emoji of emojis.slice(0, 20)) {
            const id = resolveEmojiId(emoji);
            await addReaction(session, ctx, targetId, id);
