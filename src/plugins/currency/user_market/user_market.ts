...
class UserMarketPlugin {
    ...
    private setupDatabase(): void {
        // 设置商品表
        this.ctx.model.extend('user_market_items', {
            id: 'unsigned',
            userId: 'string',
            content: 'string',
            price: 'integer',
        ...
        // 设置商品-群聊关联表
        this.ctx.model.extend('user_market_items_in_group', {
            id: 'unsigned',
            itemId: 'unsigned',
            channelId: 'string',
            createTime: 'string',
    ...
    private registerCommands(): void {
        // 私聊指令：上架商品
        this.ctx.command('market.sell <price:number>', '上架一条消息到商城（仅私聊）', {authority: 1})
            .action(async ({session}, price) => {
                ...
                try {
                    // 创建商品记录
                    const result = await this.ctx.database.create('user_market_items', {
                        userId,
                        content,
                        price,
                        createTime
        ...
        // 私聊指令：彻底删除商品
        this.ctx.command('market.delete <itemId:number>', '彻底删除你上架的商品（仅私聊）', {authority: 1})
            .action(async ({session}, itemId) => {
                ...
                try {
                    ...
                    // 删除商品-群聊关联
                    await this.ctx.database.remove('user_market_items_in_group', {itemId});

                    // 删除商品
                    await this.ctx.database.remove('user_market_items', {id: itemId});
        ...
        // 群聊指令：查看当前群聊的商城内容
        this.ctx.command('market.list', '查看当前群聊的商城内容', {authority: 1})
                ...
                try {
                    ...
                    const itemList = await Promise.all(
                        items.map(async (item, index) => {
                            const sellerName = await getUserName(this.ctx, session, item.userId);
        ...
        // 群聊指令：添加商品到当前群聊
        this.ctx.command('market.add <itemId:number>', '将你上架的商品添加到当前群聊（仅上传者和管理员可用）', {authority: 1})
            .action(async ({session}, itemId) => {
                // 只允许在群聊中使用
                if (!session.guildId) {
                    return '该命令只能在群聊中使用！';
                ...
                try {
                    ...
                    // 添加商品到群聊
                    await this.ctx.database.create('user_market_items_in_group', {
                        itemId,
                        channelId,
        ...
        // 群聊指令：从当前群聊移除商品
        this.ctx.command('market.remove <itemId:number>', '从当前群聊移除商品（仅上传者和管理员可用）', {authority: 1})
            .action(async ({session}, itemId) => {
                // 只允许在群聊中使用
                if (!session.guildId) {
                    return '该命令只能在群聊中使用！';
                ...
                try {
                    ...
                    // 从群聊中移除商品
                    await this.ctx.database.remove('user_market_items_in_group', {itemId, channelId});
        ...
        // 群聊指令：购买商品
        this.ctx.command('market.buy <itemId:number>', '购买当前群聊商城中的商品', {authority: 1})
            .action(async ({session}, itemId) => {
                // 只允许在群聊中使用
                if (!session.guildId) {
                    return '该命令只能在群聊中使用！';
                ...
                try {
                    ...
                    if (session.onebot) {
                        try {
                            const friendList = await session.onebot.getFriendList();
                            isFriend = friendList.some(friend => friend.user_id.toString() == userId);
                        } catch (error) {
                    ...
                    // 增加卖家星币
                    await StarCoinHelper.addUserStarCoin(this.ctx, item.userId, channelId, actualAmount);

                    // 发送商品内容给买家
                    if (session.onebot) {
                        try {
                            await session.onebot.sendPrivateMsg(userId, `🎉 你成功购买了商品ID ${itemId}！\n支付：${item.price} 星币\n手续费：${fee} 星币\n实际支付给卖家：${actualAmount} 星币\n商品内容：`);
                            await session.onebot.sendPrivateMsg(userId, item.content);
                        } catch (error) {
                            this.ctx.logger.warn('发送商品内容给买家失败:', error);
                        }
                    }

                    // 获取卖家名称
                    const sellerName = await getUserName(this.ctx, session, item.userId);
                    const buyerName = await getUserName(this.ctx, session, userId);
