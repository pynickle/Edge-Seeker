...
export function red_packet(ctx: Context, config: Config) {
    ...
    // 发红包命令
    ctx.command('redpacket <amount:number> <count:number>', '发送红包').action(
        async ({ session }, amount: number, count: number) => {
            ...
            try {
                ...
                const confirmed = await confirmationManager.createConfirmation(
                    ctx,
                    session,
                    config.red_packet.confirmationTimeout
                );
                if (!confirmed) {
                    return '❌ 操作已取消或超时';
                }
    ...
    ctx.command('grab <packetId:number>', '领取红包')
        ...
        .action(async ({ session }, packetId: number) => {
            ...
            // 查找红包
            const packets = await ctx.database.get('red_packets', {
                id: packetId,
            });
            if (packets.length === 0) {
                return '❌ 红包不存在或已被删除';
    ...
    // 查询红包状态命令
    ctx.command('redpacket.stats <packetId:number>', '查询红包状态').action(
        async ({ session }, packetId: number) => {
            const channelId = session.channelId;
        ...
        async () => {
            ...
            for (const packet of expiredPackets) {
                ...
                if (packet.remainingAmount > 0) {
                    // 记录日志，但不实际退还
                    ctx.logger.info(
                        `红包 ${packet.id} 已过期，剩余 ${packet.remainingAmount} 星币未领取`
                    );
                }
            }
        },
