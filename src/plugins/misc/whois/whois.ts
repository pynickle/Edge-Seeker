...
export function whois(ctx: Context) {
    ctx.command('whois <domain:string>', '查询域名 whois 信息').action(
        async ({ session }, domain) => {
            try {
                if (!domain) {
                ...
                output += mainWhois['Registrar IANA ID']
                    ? `注册商 IANA ID: ${mainWhois['Registrar IANA ID']}\n`
                    : '';
                output += mainWhois['registrar organization-loc']
                    ? `注册商本地化组织名称：${mainWhois['registrar organization-loc']}\n`
                    : '';
                output += mainWhois['Registrar WHOIS Server']
                    ? `WHOIS 服务器：${mainWhois['Registrar WHOIS Server']}\n`
                    : '';
                ...
                output += mainWhois['Registrant Email']
                    ? `注册人邮箱：${mainWhois['Registrant Email']}\n`
                    : '';
                output += mainWhois['Registrant Name']
                    ? `注册人：${mainWhois['Registrant Name']}\n`
                    : '';
                output += mainWhois['Registrant Organization']
                    ? `注册人组织：${mainWhois['Registrant Organization']}\n`
                    : '';
                output += mainWhois['Registrant Country']
                    ? `注册人国家：${mainWhois['Registrant Country']}\n`
                    : '';

                output += mainWhois['Registry Domain ID']
