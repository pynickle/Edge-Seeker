...
export function auto_red_packet(ctx: Context, cfg: Config) {
    ...
    // 检查是否应该发送红包
    const shouldSendRedPacket = async (
        channelId: string,
        minInterval: number,
        maxInterval: number
    ): Promise<boolean> => {
            ...
            for (const config of channelConfigs) {
                ...
                if (
                    ...
                    await bot.sendMessage(
                        channelId,
                        `🎁 系统福利红包来啦！${count} 个共 ${amount} 星币的红包！\n发送「抢红包 ${packet.id}」来领取吧~\n有效期：${expiryHours}小时`
                    );

                    // 更新上次发送时间
                    await updateLastSendTime(channelId, now);

                    ctx.logger('auto-red-packet').info(
                        `已在频道 ${channelId} 发送自动红包，ID：${packet.id}`
                    );
                }
            }
        } catch (error) {
